-- ============================================================
-- MIGRATION: Bondada employee referral fields
--
-- Adds the 3 new fields captured on Step 1 (Personal Details)
-- of the scholarship form:
--   - "Are you referred by a Bondada employee?" (Yes/No)
--   - Employee Name   (only when Yes)
--   - Employee ID     (only when Yes)
--
-- Safe to re-run: each ALTER is guarded by an information_schema
-- check, so running this twice against the same database is a
-- no-op the second time. Run this ONCE against your existing
-- (already-deployed) database. Fresh installs don't need this
-- file at all — 00_schema.sql already includes these columns.
--
-- Usage:
--   mysql -u <user> -p <database_name> < 05_alter_bondada_employee_referral.sql
-- ============================================================

DELIMITER $$

DROP PROCEDURE IF EXISTS _bf_migrate_bondada_employee_referral $$
CREATE PROCEDURE _bf_migrate_bondada_employee_referral()
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'scholarship_applications'
          AND COLUMN_NAME = 'referred_by_bondada_employee'
    ) THEN
        ALTER TABLE scholarship_applications
            ADD COLUMN referred_by_bondada_employee TINYINT(1) NOT NULL DEFAULT 0 AFTER email,
            ADD COLUMN employee_name VARCHAR(150) NULL AFTER referred_by_bondada_employee,
            ADD COLUMN employee_id VARCHAR(50) NULL AFTER employee_name;
    END IF;
END $$

DELIMITER ;

CALL _bf_migrate_bondada_employee_referral();
DROP PROCEDURE IF EXISTS _bf_migrate_bondada_employee_referral;

-- ============================================================
-- Re-create sp_scholarship_application_create with the 3 new
-- IN params. Stored procedures aren't altered by ALTER TABLE,
-- so this replaces the old 22-param version with the new
-- 25-param version. Safe to re-run (DROP IF EXISTS first).
-- ============================================================
